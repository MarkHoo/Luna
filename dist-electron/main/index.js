...
const _NetworkDetector = class _NetworkDetector {
    ...
    const results = await Promise.all(
      this.sources.map(async (source) => {
        ...
        try {
          ...
          await axios.head(source.baseUrl, { timeout: 5e3 });
  ...
  static async testSource(sourceId) {
  ...
  static getDefaultSource(sources) {
    const available = sources.find((s) => s.available && s.id !== "local");
    return (available == null ? void 0 : available.id) || "local";
  }
};
_NetworkDetector.sources = [
...
let NetworkDetector = _NetworkDetector;
class DownloadManager {
  constructor() {
    this.tasks = /* @__PURE__ */ new Map();
    this.activeDownloads = /* @__PURE__ */ new Map();
    this.modelsDir = path$1.join(electron.app.getPath("userData"), "models");
    if (!fs$1.existsSync(this.modelsDir)) {
      fs$1.mkdirSync(this.modelsDir, { recursive: true });
    }
  }
  async startDownload(modelId, source, url2, onProgress) {
    const taskId = v4();
    const fileName = url2.split("/").pop() || `${modelId}.gguf`;
    const savePath = path$1.join(this.modelsDir, fileName);
    const task = {
      id: taskId,
      modelId,
      source,
  ...
  async cancelDownload(taskId) {
    ...
    if (task) {
      ...
      if (fs$1.existsSync(task.savePath)) {
        fs$1.unlinkSync(task.savePath);
      }
    }
...
class LlamaManager {
  ...
  findLlamaBinary() {
    const possiblePaths = [
      path$1.join(electron.app.getPath("userData"), "llama", "llama-server"),
      path$1.join(electron.app.getPath("userData"), "llama", "llama-server.exe"),
  ...
  async startServer(modelPath, params, onLog) {
    ...
    try {
      this.process = child_process.spawn(this.llamaBinaryPath, args, {
...
class FileManager {
  constructor() {
    ...
    this.configPath = path$1.join(userDataPath, "config.json");
    this.modelsConfigPath = path$1.join(userDataPath, "models.json");
    this.modelsDir = path$1.join(userDataPath, "models");
  ...
  ensureDirectories() {
    ...
    if (!fs$1.existsSync(this.modelsConfigPath)) {
      fs$1.writeFileSync(this.modelsConfigPath, JSON.stringify({ models: [] }, null, 2));
    }
  }
  getLocalModels() {
    try {
      ...
      const data = JSON.parse(fs$1.readFileSync(this.modelsConfigPath, "utf-8"));
      return data.models || [];
    } catch {
      return [];
    }
  ...
  async deleteModel(modelPath) {
    try {
      if (fs$1.existsSync(modelPath)) {
        fs$1.unlinkSync(modelPath);
      }
      const models = this.getLocalModels();
      const updated = models.filter((m) => m.path !== modelPath);
      fs$1.writeFileSync(this.modelsConfigPath, JSON.stringify({ models: updated }, null, 2));
      return true;
    } catch (error) {
  ...
  getSettings() {
    try {
      ...
      const data = JSON.parse(fs$1.readFileSync(this.configPath, "utf-8"));
  ...
  saveSettings(settings) {
    try {
      ...
      fs$1.writeFileSync(this.configPath, JSON.stringify(updated, null, 2));
      return true;
    } catch (error) {
  ...
  getAvailableDiskSpace() {
    const stat = fs$1.statfsSync(this.modelsDir);
...
electron.ipcMain.handle("network:select-source", async (_, source) => {
  return await NetworkDetector.testSource(source);
});
electron.ipcMain.handle("download:start", async (event, modelId, source, url2) => {
  return await downloader.startDownload(modelId, source, url2, (progress) => {
    mainWindow == null ? void 0 : mainWindow.webContents.send("download:progress", progress);
  });
});
electron.ipcMain.handle("download:pause", async (_, taskId) => {
  return await downloader.pauseDownload(taskId);
});
electron.ipcMain.handle("download:resume", async (_, taskId) => {
  return await downloader.resumeDownload(taskId);
});
electron.ipcMain.handle("download:cancel", async (_, taskId) => {
  return await downloader.cancelDownload(taskId);
});
electron.ipcMain.handle("llama:start", async (_, modelPath, params) => {
  return await llamaManager.startServer(modelPath, params, (log) => {
    mainWindow == null ? void 0 : mainWindow.webContents.send("llama:log", log);
  });
});
...
electron.ipcMain.handle("file:get-models", async () => {
  return fileManager.getLocalModels();
});
electron.ipcMain.handle("file:delete-model", async (_, modelPath) => {
  return fileManager.deleteModel(modelPath);
});
electron.ipcMain.handle("file:get-settings", async () => {
  return fileManager.getSettings();
});
electron.ipcMain.handle("file:save-settings", async (_, settings) => {
  return fileManager.saveSettings(settings);
});
electron.ipcMain.handle("file:select-path", async () => {
